...
只管理 canonical IPv4 来源目录中的受支持名称：`YYYY-MM-DD.log`、`YYYY-MM-DD.log-YYYYMMDD[-HHMMSS][.gz]` 和旧数字后缀 `.log.N[.gz]`。保留日期始终取文件名最前面的服务器本地接收日期，不用 mtime，也不用发送设备自报时间。Python 查找、压缩和保留边界统一使用相同本地日历日期；内部事件依然 UTC。为兼容不同本地时区的旧文件，删除阈值多留一天；保留边界内的数据即使一天切分 80 份也不会提前删除。
...
- 恢复：仅对维护器自行暂停的 receiver，在空闲超过 `min_free_mb+128 MiB` 且 syslog 小于预算 75% 时自动启动；清理过期归档后会再次判断，允许同一轮恢复。管理员已手动停止的服务不被 guard 取得自动恢复权。启动失败至少等待 300 秒再试。
- 暂停状态在 `state/syslog-storage.json`，API `/syslog` 可见。ExecCondition 防止已标记的 receiver 因重启再次写满磁盘。空间完全耗尽导致标记写失败时仍执行 stop 并输出 STORAGE_PRESSURE 至 journal；无法可靠确认暂停所有权时必须由管理员检查恢复，不能凭标记缺失自动 start。损坏标记保留并形成 manual_intervention 错误，需修复后用 --clear-error 解除。
...
可选 `expected_sources`，默认空数组。来源首次观测无证据为 UNKNOWN；首次/receiver重启会进行一次有界文件基线读取，即使新进程counter为0也可恢复真实last_received_at；之后仅计数变化时尝试读取接收服务器本地日期的当天/前一天文件最多各 64 KiB 的真实接收时间；若不能读到则使用统计变化的观测上界，并标明精度。不能因为启动后看见一个旧累计值就断言正在接收。跨 observer/receiver 重启保留 last seen 元数据但不把旧 counter 跨进程拼接为精确总数。统计容量限制/淘汰时标记计数范围，不影响原始日志接收。
